...
class StartTasks(Resource):
    ...
    def post(self):
        ...
        rp.add_argument("writein_switch", type=parser.modul_parameter, required=True, location="json")
        rp.add_argument("data_slices", type=parser.modul_parameter, required=True, location="json")
        ...
        rp.add_argument("segmentation_phy", type=parser.modul_parameter, required=True, location="json")
        ...
        rp.add_argument("segmentation_sig", type=parser.modul_parameter, required=True, location="json")
        rp.add_argument("outer_sig", type=parser.modul_parameter, required=True, location="json")
        rp.add_argument("inner_sig", type=parser.modul_parameter, required=True, location="json")
        rp.add_argument("ball_sig", type=parser.modul_parameter, required=True, location="json")
        ...
        rp.add_argument("epochs_cnn", type=parser.modul_parameter, required=True, location="json")
        ...
        writein_switch = ret["writein_switch"]
        data_slices = ret["data_slices"]
        ...
        segmentation_phy = ret["segmentation_phy"]
        ...
        segmentation_sig = ret["segmentation_sig"]
        outer_sig = ret["outer_sig"]
        inner_sig = ret["inner_sig"]
        ball_sig = ret["ball_sig"]
        ...
        # 新流程框架
            ...
            if modul1 == 1:
                ...
                use_data, use_data_label = bearing_one_main(segmentation_phy=segmentation_phy, length=length,
                                                            shift=shift,
                                                            Kb=Kb_phy,
                                                            d=d_phy,
                                                            Nb=Nb,
                ...
                print("物理模型label形状为:", use_data_label.shape)
                ...
                print("shift", shift)
                print("length", length)
                print("segmentation_sig", segmentation_sig)
                use_data, use_data_label = signal_main(outer_sig=outer_sig, inner_sig=inner_sig, ball_sig=ball_sig,
                                                       segmentation_sig=segmentation_sig, length=length, shift=shift,
                                                       D=D_sig,
                                                       di=Di_sig,
                                                       do=Do_sig,
                                                       Z=z_sig,
                ...
                use_data, use_data_label = CRWU_read(file_lists=file_lists,
                                                     ...
                                                     save_path=save_path)
                # 数据打乱
                from utils.read_data import data_breakdown
                use_data, use_data_label = data_breakdown(use_data, use_data_label)
            ...
            elif modul5 and not (modul1 + modul2 + modul3 + modul4):
                pass
            # 如果模块一为被选择
            else:
                print("使用上传数据!")
                ...
                print("writein_switch", writein_switch)
                use_data = read_time_domain_signal(data_path, writein_switch=writein_switch)
                print("数据读取形状为:", use_data.shape)


                # use_data = use_data[:, :10]
                # print("数据读取形状为:", use_data.shape)
                # except:
                #     use_data = None

                # 1.2 读取label:读取数据label:允许不上传label,并且label与data必须匹配 and (modul2 or modul4 or modul5)
                if use_data is not None:
                    try:
                        ...
                        use_data_label = read_time_domain_signal(label_path, writein_switch=writein_switch)
                        print("读取label为:", use_data_label.shape)
                        ...
                        # 数据切片
                        from moduls.utils.read_data import data_slice
                        use_data, use_data_label = data_slice(use_data, use_data_label, slice_str=data_slices)
                        print("数据切片形状为:", use_data.shape)

                    else:
                        # 判断数据与label形状是否匹配
                        # if modul2 or modul4 or modul5:
                        if use_data.shape[0] != use_data_label.shape[0]:
                            # 当label非必要时:不报错,并且label重置为None,label必要时,报错
                            # if modul4:
                            abort(400, "ERROR: The data and label are not suitable. Please check the data file again.")
                            # else:
                            #     use_data_label = None
            ...
            # 3 模块二
            # 模块二运行条件判断
            if modul2 and not (use_data is not None):
                abort(400, "ERROR: No data found. Please check the upload file or procedure configuration.")
            if modul2:
                ...
                # 情况一:有label剧增
                if (use_data_label is not None):
                    use_data, use_data_label = data_augmentation_main(use_data,
                                                                      use_data_label,
                                                                      switch=modul2,
                                                                      GAN_num=GAN_num,
                                                                      Z_dim=GAN_Z_dim,
                                                                      image_transform_multi=image_multi,
                                                                      deltax=image_deltax,
                                                                      deltay=image_deltay,
                                                                      rot=image_rot,
                                                                      snr=image_snr,
                ...
                # 情况二:无label剧增
                    ...
                    use_data, _ = data_augmentation_main(use_data,
                                                         None,
                                                         switch=modul2,
                                                         GAN_num=GAN_num,
                                                         Z_dim=GAN_Z_dim,
            ...
            # 4 模块三
            # 模块三运行条件判断:
            if modul3 and not (use_data is not None):
                abort(400, "ERROR: No data found. Please check the upload file or procedure configuration.")
            ...
            if modul3:
                ...
                # print("mean_filt_length:", mean_filt_length)
                use_data = data_denoise_main(use_data,
                                             ...
                                             output_file=output_files,
                                             output_image=output_images)
                print("模块3输出数据形状为:", use_data.shape)

            # 5 模块四
            # 模块四运行条件判断:
            if modul4 and ((use_data is None) or (use_data_label is None)):
                abort(400, "ERROR: No data or label found. Please check the upload file or procedure configuration.")
            if modul4:
                ...
                print("t_features", t_features)
                print("f_features", f_features)
                ...
                try:
                    ...
                    data_path = os.path.join(upload_file, "upload")
                    data_path = os.path.join(data_path, file_name)
                    data_SBN0 = read_time_domain_signal(data_path, writein_switch=0)
                    print("data_SBN0", data_SBN0.shape)
                except:
                    data_SBN0 = None
                # SBN0 = (1, xxxx)
                # data_SBN0 = use_data[0,:].reshape((1, -1))

                # print("特征提取", data_SBN0.shape)
                ...
                if feature_selection_method:
                    ...
                    use_data, use_data_label = Features_Selection_main(use_data,
                                                                       use_data_label,
                                                                       name_list,
            ...
            if modul5 and not (modul1 + modul2 + modul3 + modul4):
                try:
                    ...
                    x_train = read_time_domain_signal(traindata_path, writein_switch=writein_switch)
                    y_train = read_time_domain_signal(trainlabel_path, writein_switch=writein_switch)
                    x_test = read_time_domain_signal(testdata_path, writein_switch=writein_switch)
                    y_test = read_time_domain_signal(testlabel_path, writein_switch=writein_switch)
            ...
            elif modul5 and (not (use_data is not None) or not (use_data_label is not None)):
                abort(400, "ERROR: No data or label found. Please check the upload file or procedure configuration.")
            ...
            # 使用流程中数据
                ...
                if not rul_pre:
                    if use_data.shape[0] < 100:
                        abort(400,
                              f"ERROR: The number of training samples cannot be less than 100. The current sample size is {use_data.shape[0]}.")
                # 去NAN
                use_data = np.nan_to_num(use_data)
                ...
                if not rul_pre:
                    ...
                    if list(set(list(np.reshape(use_data_label, (-1,))))).__len__() < 2:
                        abort(400, f"ERROR: The number of classes has to be greater than one; got {nums_of_labels} class.")
                ...
                try:
                    ...
                    abort(400,
                          f"ERROR: Data splitting error. Please check the data. The current data shape is {use_data.shape}. The current label shape is {use_data_label.shape}.")
            ...
            if modul5:
                ...
                # 判断label是否可以用于 故障诊断 或 RUL预测
                if rul_pre:
                    # RUL预测时,label取值只能在0-1之间
                    print("y_train", y_train)
                    print("y_train.max()", y_train.max())
                    if y_train.max() > 1 or y_train.min() < 0:
                        abort(400, "ERROR: The range of labels in the RUL prediction is [0, 1].")
                else:
                    # 故障诊断时,label不能有小数
                    ...
                    print("y_train", y_train)
                    print("y_train.max()", y_train.max())
                    if list(set(list(np.reshape(y_train, (-1,))))).__len__() < 2:
                        abort(400,
                              f"ERROR: There is a problem with the data label in the fault diagnosis. This may be because "
                              f"the number of classes is less than 2, or there are floating point numbers in the label.")
                ...
                save_data_func(x_train, output_file=output_files, save_path=save_path, file_name="traindata", index_label="(sample_nums, sample_length)")
                save_data_func(y_train, output_file=output_files, save_path=save_path, file_name="trainlabel", index_label="(sample_nums, sample_labels)")
                save_data_func(x_test, output_file=output_files, save_path=save_path, file_name="testdata", index_label="(sample_nums, sample_length)")
                save_data_func(y_test, output_file=output_files, save_path=save_path, file_name="testlabel", index_label="(sample_nums, sample_labels)")

                if opt_algorithm == 1:
                    option = "PSO"
                    if pso_pop_size < 1:
                        print("pso_pop_size", pso_pop_size)
                        abort(400, "ERROR: The parameter PSO pop size must be a positive integer and greater than 2.")
                    if pso_max_itr < 1:
                        abort(400, "ERROR: The parameter PSO maximum iteration must be a positive integer and greater than 2.")
                elif opt_algorithm == 2:
                    option = "GA"
                    print("ga_max_itr", ga_pop_size)
                    if ga_pop_size < 2:
                        abort(400, "ERROR: The parameter GA pop size must be a positive integer and greater than 2.")
                    if ga_max_itr < 2:
                        abort(400, "ERROR: The parameter GA maximum iteration must be a positive integer and greater than 2.")
                elif opt_algorithm == 3:
                    option = "SA"
                    if sa_max_itr < 2:
                        abort(400, "ERROR: The parameter SA maximum iteration must be a positive integer and greater than 2.")
                    if sa_alpha <= 0 or sa_alpha >= 1:
                        abort(400, "ERROR: The value range of parameter SA alpha is (0, 1).")
                else:
                    option = None
                ...
                if modul5 == 1:
                    ...
                    report.word_knn(save_path=save_path, output_files=output_files, output_images=output_images,
                    ...
                    opt_params = f"The optimal parameters after optimization are:<br>K : {params_dict['K']}<br>weights : {params_dict['weights']}"

                elif modul5 == 2:
                    from moduls.ml.svm_main import SVM

                    # print(c_svm, gamma_svm)
                    y_pre, params_dict = SVM(x_train, x_test, y_train, y_test,
                                             rul_pre=rul_pre,
                                             C=c_svm,
                                             gamma=gamma_svm,
                                             pso_part_num=pso_pop_size,
                                             ...
                                             save_path=save_path)
                    """params = [C, gamma]"""
                    report.word_svm(save_path=save_path, output_files=output_files, output_images=output_images,
                                    train_data=x_train, train_label=y_train, test_data=x_test, test_label=y_test,
                                    opt_algorithm=opt_algorithm, rul_pre=rul_pre,
                                    C=params_dict['C'], gamma=params_dict['gamma'],
                                    pso_pop_size=pso_pop_size, pso_max_itr=pso_max_itr,
                                    ga_pop_size=ga_pop_size, ga_max_itr=ga_max_itr,
                                    sa_alpha=sa_alpha, sa_max_itr=sa_max_itr,
                                    accuracy=params_dict['accuracy'],
                                    )
                    opt_params = f"The optimal parameters after optimization are:<br>C : {params_dict['C']}<br>Gamma : {params_dict['gamma']}"
                elif modul5 == 3:
                    from moduls.ml.DT_main import DT

                    y_pre, params_dict = DT(x_train, x_test, y_train, y_test,
                                            rul_pre=rul_pre,
                                            ...
                                            output_image=output_images,
                                            save_path=save_path)
                    report.word_dt(save_path=save_path, output_files=output_files, output_images=output_images,
                                   train_data=x_train, train_label=y_train, test_data=x_test, test_label=y_test,
                                   opt_algorithm=opt_algorithm, rul_pre=rul_pre,
                                   max_depth=params_dict['max_depth'], max_leaf_nodes=params_dict['max_leaf_nodes'],
                                   pso_pop_size=pso_pop_size, pso_max_itr=pso_max_itr,
                                   ga_pop_size=ga_pop_size, ga_max_itr=ga_max_itr,
                                   sa_alpha=sa_alpha, sa_max_itr=sa_max_itr,
                                   accuracy=params_dict['accuracy'],
                    ...
                    opt_params = f"The optimal parameters after optimization are:<br>Max depth : {params_dict['max_depth']}<br>Max leaf nodes : {params_dict['max_leaf_nodes']}"
                elif modul5 == 4:
                    from moduls.ml.RF_main import RF

                    y_pre, params_dict = RF(x_train, x_test, y_train, y_test,
                                            rul_pre=rul_pre,
                    ...
                    report.word_rf(save_path=save_path, output_files=output_files, output_images=output_images,
                                   train_data=x_train, train_label=y_train, test_data=x_test, test_label=y_test,
                                   opt_algorithm=opt_algorithm, rul_pre=rul_pre,
                                   max_depth=params_dict['max_depth'], max_leaf_nodes=params_dict['max_leaf_nodes'],
                    ...
                    opt_params = f"The optimal parameters after optimization are:<br>Max depth : {params_dict['max_depth']}" \
                                 f"<br>Max leaf nodes : {params_dict['max_leaf_nodes']}" \
                                 f"<br>Number of estimators : {params_dict['n_estimators']}"
                elif modul5 == 5:
                    # DBN数据处理
                    if rul_pre:
                    ...
                    y_pre, params_dict = DBN(x_train, x_test, y_train, y_test,
                                             rul_pre=rul_pre,
                                             Dropout=Dropout_dbn,
                                             ...
                                             output_image=output_images,
                                             save_path=save_path)
                    report.word_dbn(save_path=save_path, output_files=output_files, output_images=output_images,
                                    train_data=x_train, train_label=y_train, test_data=x_test, test_label=y_test,
                                    opt_algorithm=opt_algorithm, rul_pre=rul_pre,
                                    Dropout=params_dict['Dropout'],
                                    # LearningRate_RBM=params_dict['LearningRate_RBM'],
                                    LearningRate_nn=params_dict['LearningRate_nn'],
                                    pso_pop_size=pso_pop_size, pso_max_itr=pso_max_itr,
                                    ga_pop_size=ga_pop_size, ga_max_itr=ga_max_itr,
                                    sa_alpha=sa_alpha, sa_max_itr=sa_max_itr,
                                    accuracy=params_dict['accuracy'],
                    ...
                    opt_params = f"The optimal parameters after optimization are:<br>Dropout : {params_dict['Dropout']}<br>Learning rate : {params_dict['LearningRate_nn']}"
                elif modul5 == 6:
                    # AE 数据处理
                    if rul_pre:
                        size_train = x_train.shape[0]
                    ...
                    y_pre, params_dict = AE(x_train, x_test, y_train, y_test,
                                            rul_pre=rul_pre,
                                            ...
                                            output_image=output_images,
                                            save_path=save_path)
                    report.word_ae(save_path=save_path, output_files=output_files, output_images=output_images,
                                   train_data=x_train, train_label=y_train, test_data=x_test, test_label=y_test,
                                   opt_algorithm=opt_algorithm, rul_pre=rul_pre,
                                   LayerCount=params_dict['LayerCount'], epochs=params_dict['epochs'],
                                   batchSize=params_dict['batchSize'], denseActivation=params_dict['denseActivation'],
                                   pso_pop_size=pso_pop_size, pso_max_itr=pso_max_itr,
                                   ga_pop_size=ga_pop_size, ga_max_itr=ga_max_itr,
                                   sa_alpha=sa_alpha, sa_max_itr=sa_max_itr,
                                   accuracy=params_dict['accuracy'],
                    ...
                    opt_params = f"The optimal parameters after optimization are:<br>Layer count : {params_dict['LayerCount']}" \
                                 f"<br>Units1 : {params_dict['units1']}" \
                                 f"<br>Units2 : {params_dict['units2']}" \
                                 f"<br>Units3 : {params_dict['units3']}" \
                                 f"<br>Epochs : {params_dict['epochs']}" \
                                 f"<br>Batch size : {params_dict['batchSize']}" \
                                 f"<br>Dense activation : {params_dict['denseActivation']}"
                elif modul5 == 7:
                    from moduls.ml.ExtraTree_main import ET
                    y_pre, params_dict = ET(x_train, x_test, y_train, y_test,
                                            rul_pre=rul_pre,
                                            ...
                                            output_image=output_images,
                                            save_path=save_path)
                    report.word_et(save_path=save_path, output_files=output_files, output_images=output_images,
                                   train_data=x_train, train_label=y_train, test_data=x_test, test_label=y_test,
                                   opt_algorithm=opt_algorithm, rul_pre=rul_pre,
                                   max_depth=params_dict['max_depth'], max_leaf_nodes=params_dict['max_leaf_nodes'],
                                   n_estimators=params_dict['n_estimators'],
                    ...
                    y_pre, params_dict = Bagging(x_train, x_test, y_train, y_test,
                                                 rul_pre=rul_pre,
                                                 ...
                                                 output_image=output_images,
                                                 save_path=save_path)
                    report.word_bagging(save_path=save_path, output_files=output_files, output_images=output_images,
                                        train_data=x_train, train_label=y_train, test_data=x_test, test_label=y_test,
                                        opt_algorithm=opt_algorithm, rul_pre=rul_pre,
                    ...
                    """params = [max_depth, max_leaf_nodes, n_estimators]"""
                    opt_params = f"The optimal parameters after optimization are:<br>Max leaf nodes : {params_dict['max_leaf_nodes']}" \
                                 f"<br>Number of estimators : {params_dict['n_estimators']}"
                elif modul5 == 11:
                    ...
                    if x_train.shape[1] < 12:
                        abort(400, "ERROR: The minimum feature length of CNN is 12.")
                    # if not option and (conv_cnn > 9 or conv_cnn < 1):
                    #     abort(400, "The range of the convolution kernel is [1, 9].")
                    if not option and x_train.shape[1] < batch_size_cnn:
                        abort(400,
                              f"ERROR: The size of Batch_size cannot be greater than the sample length. The current sample length is {x_train.shape[1]}.")
                    if not option and (batch_size_cnn not in list(range(1, 512))):
                        abort(400, "ERROR: The size of Batch_size can only be in the range of [1, 16, 32, 64, 128, 256].")
                    y_pre, params_dict = CNN(x_train, x_test, y_train, y_test,
                                             rul_pre=rul_pre,
                                             dropout=dropout_cnn,
                    ...
                    report.word_cnn(save_path=save_path, output_files=output_files, output_images=output_images,
                                    train_data=x_train, train_label=y_train, test_data=x_test, test_label=y_test,
                                    opt_algorithm=opt_algorithm, rul_pre=rul_pre,
                                    dropout=params_dict['dropout'], learning_rate=params_dict['learning_rate'],
                                    batch_size=params_dict['batch_size'], epochs=params_dict['epochs'],
                                    pso_pop_size=pso_pop_size, pso_max_itr=pso_max_itr,
                                    ga_pop_size=ga_pop_size, ga_max_itr=ga_max_itr,
                                    sa_alpha=sa_alpha, sa_max_itr=sa_max_itr,
                                    accuracy=params_dict['accuracy'],
                                    )
                    opt_params = f"The optimal parameters after optimization are:<br>Dropout : {params_dict['dropout']}" \
                                 f"<br>Learning rate : {params_dict['learning_rate']}" \
                                 f"<br>Batch size : {params_dict['batch_size']}" \
                                 f"<br>Epochs : {params_dict['epochs']}"
                elif modul5 == 12:
                    # LSTM的数据处理
                    if rul_pre:
                    ...
                    y_pre, params_dict = LSTM(x_train, x_test, y_train, y_test,
                                              rul_pre=rul_pre,
                                              ...
                                              output_image=output_images,
                                              save_path=save_path)
                    report.word_lstm(save_path=save_path, output_files=output_files, output_images=output_images,
                                     train_data=x_train, train_label=y_train, test_data=x_test, test_label=y_test,
                                     opt_algorithm=opt_algorithm, rul_pre=rul_pre,
                                     lstm_count=params_dict['lstm_count'], dropoutRate=params_dict['dropoutRate'],
                                     epochs=params_dict['epochs'], batchSize=params_dict['batchSize'],
                                     denseActivation=params_dict['denseActivation'],
                                     pso_pop_size=pso_pop_size, pso_max_itr=pso_max_itr,
                                     ga_pop_size=ga_pop_size, ga_max_itr=ga_max_itr,
                                     sa_alpha=sa_alpha, sa_max_itr=sa_max_itr,
                                     accuracy=params_dict['accuracy'],
                                     )
                    opt_params = f"The optimal parameters after optimization are:<br>LSTM count : {params_dict['lstm_count']}" \
                                 f"<br>Units1 : {params_dict['units1']}" \
                                 f"<br>Units2 : {params_dict['units2']}" \
                                 f"<br>Units3 : {params_dict['units3']}" \
                                 f"<br>Dropout : {params_dict['dropoutRate']}" \
                                 f"<br>Epochs : {params_dict['epochs']}" \
                                 f"<br>Batch size : {params_dict['batchSize']}" \
                                 f"<br>Dense activation : {params_dict['denseActivation']}"

                save_data_func(y_pre, output_file=output_files, save_path=save_path, file_name="pred_label",
                               index_label="(sample_nums, sample_labels)")
        # 5 将所有保存的文件打包
        from utils.zip_dir import zip_dir
...
class Download(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            dowmload_file = None
            abort(400, "ERROR: No downloadable files found!")
...
def process_validation(modul1, modul2, modul3, modul4, modul5):
    ...
    if modul2 == 3 and (modul1 + modul3 + modul4 + modul5):
        abort(400, "ERROR: In the current version, the Monte Carlo method can only be used alone.")
