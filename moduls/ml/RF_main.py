...
from moduls.ml.py.RandomForest import get_score
...
def run_dt_pso(x_train, x_test, y_train, y_test, var_size, rul_pre=False,
    ...
    pso = PSO(get_score, x_train=x_train, x_test=x_test, y_train=y_train, y_test=y_test, part_num=part_num,
              ...
              var_size=var_size, net="RF", save_path=save_path)
    param_dict = pso.run()
...
def run_dt_sa(x_train, x_test, y_train, y_test, var_size, rul_pre=False,
    ...
    sa = SA(get_score, x_train=x_train, x_test=x_test, y_train=y_train, y_test=y_test,
            ...
            alpha=alpha, max_iter=max_iter, var_size=var_size, net="RF",
            save_path=save_path)
...
def run_dt_ga(x_train, x_test, y_train, y_test, var_size, rul_pre=False,
    ...
    best_generation, dna_set = run(get_score, x_train=x_train, x_test=x_test,
                                   ...
                                   rul_pre=rul_pre, output_image=output_image,
                                   Model_name="RF", save_path=save_path, threshold=threshold,
...
# var_size = [[4, 20], [1, 12]]
def RF(x_train, x_test, y_train, y_test, opt_option=None, rul_pre=False,
       output_image=0,
       max_depth=20, max_leaf_nodes=600, n_estimators=30,
    ...
    # below should get from config[4, 20, 1, 12, 8, 30, 5]
    # var_size = [max_depth, max_leaf_nodes, n_estimators]
    var_size = [[4, 20], [2, 1000], [8, 30]]
    # var_size = [[4, 100], [2, 80], [8, 100]]
    ...
    if opt_option == 'PSO':
        ...
        params_dict = run_dt_sa(x_train, x_test, y_train, y_test, var_size, rul_pre=rul_pre,
...
if __name__ == '__main__':
    ...
    acc = RF(x_train, x_test, y_train, y_test, rul_pre=False,
             pso_num_itr=5, output_image=0, opt_option=option)
    print("最终准确率为：", acc)

    # results = [max_depth, max_leaf_nodes]
    selection = [4, 20, 1, 12, 8, 30]
