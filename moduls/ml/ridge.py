...
class RIDGE_Model:
    ...
    def predict(self, params):
        ...
        self.ridge = RidgeClassifierCV(alphas=alphas_to_test, store_cv_values=True)
        ...
        self.ridge.fit(self.x_test, self.y_test)  # train on train_file
        err = 1 - self.ridge.score(self.x_test, self.y_test)
        Error = 1 - self.ridge.score(self.x_train, self.y_train)  # error on test_file
    ...
    def save_result(self):
        ...
        """
        pickle.dump(self.ridge, open(self.outdir+self.filename, 'wb'))
    ...
    def get_score(self, params):
        ...
        alphas_to_test = params[0]
        alphas_to_test = np.linspace(1e-7, 1e2, num=50)  # 现在我们有50个test用的点
        # {'auto', 'svd', 'cholesky', 'lsqr', 'sparse_cg', 'sag', 'saga'},
        # self.et = RidgeClassifierCV(alphas=alphas_to_test, normalize=True, store_cv_values=True)
        # self.et = RidgeClassifier(alpha=-100, normalize=False)
        # self.et = SGDClassifier(loss="squared_loss",
        #                            fit_intercept=True,
        #                            learning_rate="optimal",
        #                            max_iter=1e5)
        # self.et = RadiusNeighborsClassifier(radius=4.8)
        # self.et = KNeighborsClassifier(n_neighbors=2)
        ## self.et = GaussianProcessClassifier()
        # self.et = AdaBoostClassifier()
        #### self.et = BaggingClassifier()
        #### self.et = ExtraTreesClassifier()

        self.ridge.fit(self.x_train, self.y_train)  # 训练模型
        pre = self.ridge.predict(self.x_test)  # 对测试集进行分类预测
        Error = 1 - self.ridge.score(self.x_test, self.y_test)  # 计算测试分类正确率

        # self.et.fit(self.x_test, self.y_test)  # 训练模型
        # pre = self.et.predict(self.x_train)  # 对测试集进行分类预测
        # Error = 1 - self.et.score(self.x_train, self.y_train)  # 计算测试分类正确率
