...
class KNN_Model:
    ...
    def get_score(self, params, rul_pre=False):
        ...
        if rul_pre:
            self.knn = KNeighborsRegressor(n_neighbors=k, weights=weights, metric='minkowski', p=2)
            self.knn.fit(self.x_train, self.y_train)  # 训练模型
            pre = self.knn.predict(self.x_test)  # 对测试集进行分类预测
            score = r2_score(self.y_test, pre)
            Error = 1 - score

        else:
            self.knn = KNeighborsClassifier(n_neighbors=k, weights=weights, metric='minkowski', p=2)
            self.knn.fit(self.x_train, self.y_train)  # 训练模型
            pre = self.knn.predict(self.x_test)  # 对测试集进行分类预测
            Error = 1 - self.knn.score(self.x_test, self.y_test)  # 计算测试分类正确率
