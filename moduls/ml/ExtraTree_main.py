...
def run_ET_sa(x_train, x_test, y_train, y_test, var_size, output_image=0,
              rul_pre=False, initial_temp=500, final_temp=1,
...
def run_ET_ga(x_train, x_test, y_train, y_test, var_size, output_image=0,
              rul_pre=False, threshold=100.9960, dna_size=9, pop_size=6,
    ...
    best_generation, dna_set = run(knn, var_size=var_size, Model_name="ET", save_path=save_path, threshold=threshold,
                                   dna_size=dna_size, pop_size=pop_size,
                                   cross_rate=cross_rate, mutation_rate=mutation_rate,
                                   n_generations=n_generations,
                                   output_image=output_image, rul_pre=rul_pre)
...
def ET(x_train, x_test, y_train, y_test, rul_pre=False, opt_option=None,
       output_image=0,
       max_depth=28, max_leaf_nodes=10000, n_estimators=2000,
...
if __name__ == '__main__':
    ...
    acc = ET(x_train, x_test, y_train, y_test, rul_pre=True, opt_option=option,
             output_image=0, save_path=save_path, pso_num_itr=5, pso_part_num=6)
    print("最终准确率为：", acc)
