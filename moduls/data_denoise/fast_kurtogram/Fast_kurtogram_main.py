...
# 求得关键参数fc和bw
def Fast_Kurtogram(x, nlevel=6, Fs=12000, Kurtosis_figure=False,output_image=0,NFIR=16, fcut=0.4,
                   opt1=1, opt2=1,save_path='./'):
    """
    ...
    :param Kurtosis_figure: If ``True`` outputs debugging information
    ...
    N2 = np.log2(N) - 7
    if N2<1:
        abort(400, "The length of input data is too short. The minimum length is %d." % pow(2,8))
    if nlevel > N2 or nlevel<=1 :
        # abort(400, "Max decomposition levels is %d " % N2)
        abort(400, "Decomposition levels is between (1,%d]. " % N2)
    ...
    if opt1 == 1:
        ...
        if opt2 == 1:  # kurtosis of the complex envelope
            Kwav = K_wpQ(x, h, g, h1, h2, h3, nlevel, Kurtosis_figure, 'kurt2')
        else:  # variance of the envelope magnitude
            Kwav = K_wpQ(x, h, g, h1, h2, h3, nlevel, Kurtosis_figure, 'kurt1')

        # keep positive values only!
        # Kwav[Kwav <= 0] = 0
        Level_w = np.arange(1, nlevel + 1)#MATLAB里的lvl
        ...
        #计算K 来在图上显示出
        K = K_wpQ(x, h, g, h1, h2, h3, nlevel, Kurtosis_figure, 'kurt2', level=0)
        Kurtosis_max = np.max(K)
        # print(Kurtosis_max)
        if Kurtosis_figure:
            plotKurtogram(Kurtosis_max,Kwav, freq_w, nlevel, Level_w, Fs, fi, level_index,output_image,save_path)
...
def plotKurtogram(Kurtosis_max,Kwav, freq_w, nlevel, Level_w, Fs,fi, I,output_image,save_path):
    """
    Plots the kurtogram.
    ...
    plt.imshow(Kwav,aspect='auto', extent=(0, freq_w[-1], range(2 * nlevel)[-1],range(2 * nlevel)[0]),
               interpolation='none', cmap=plt.cm.rainbow)
    plt.yticks(range(2 * nlevel), np.round(Level_w * 10) / 10)
    plt.plot(Fs * fi, I, 'yo')
    plt.xlabel("Frequency (Hz)")
    plt.ylabel("Level k")
    plt.title("K_max=%.2f, at Level %.1f, Bw=%.2f Hz, fc=%.2f Hz" %
              (Kurtosis_max,np.round(10 * Level_w[I]) / 10, Fs * 2 ** (-(Level_w[I] + 1)), Fs * fi))
    plt.colorbar(label='Kurtosis')

    if output_image == 0:
        file_name1 = "%s.png" % "Kurtosis"
        path1 = os.path.join(save_path, file_name1)
        plt.savefig(path1)
    elif output_image == 1:
        file_name1 = "%s.png" % "Kurtosis"
        file_name2 = "%s.jpg" % "Kurtosis"
        path1 = os.path.join(save_path, file_name1)
...
def K_wpQ(x, h, g, h1, h2, h3, nlevel, Kurtosis_figure, opt, level=0):#计算峭度矩阵
    """
    ...
    if level == 0:
        ...
        level = nlevel
    x = x.ravel()
    KD, KQ = K_wpQ_local(x, h, g, h1, h2, h3, nlevel, Kurtosis_figure, opt, level)
...
def K_wpQ_local(x, h, g, h1, h2, h3, nlevel, Kurtosis_figure, opt, level):#计算每层的峭度
    """
    ...
    if level > 1:
        Ka, KaQ = K_wpQ_local(a, h, g, h1, h2, h3, nlevel, Kurtosis_figure, opt,
                              level - 1)

        Kd, KdQ = K_wpQ_local(d, h, g, h1, h2, h3, nlevel, Kurtosis_figure, opt,
                              level - 1)
...
def Find_wav_kurt(x, h, g, h1, h2, h3, nlevel, Sc, Fr, Fs=1, Kurtosis_figure=False):
    """
...
# 主函数
def FK_filterdesign(x,nlevel=6, Fs=12000,mode='lowpass',order=8,Kurtosis_figure=True,save_path='./',output_file=0,output_image=0):
    '''
    主函数
    :param x: input time domain signal,一维数组，例如(10001,)
    :param nlevel:number of decomposition levels
    :param Fs: sampling frequency
    :param mode: 'lowpass','bandpass','highpass'
    :param order: order of filter
    :param save_path: path to save
    :param Kurtosis_figure: ob to show the kurtosis-image, True or False
  ...
  # 设计带宽滤波器
    fc,bw= Fast_Kurtogram(x, nlevel, Fs, Kurtosis_figure,output_image, NFIR=16, fcut=0.4,opt1=1, opt2=2,save_path=save_path)
    Wn1 = (fc - bw / 2+0.000001)/(Fs/2)
    # Wn1=0.00001
    Wn2 = (fc + bw / 2)/(Fs/2)-0.0000001
    if order <= 0:
        abort(400, "ERROR: The order must be a positive integer.")
    if (mode == 'bandpass'):
        b, a = si.butter(order, [Wn1, Wn2], 'band')  # 配置滤波器 8 表示滤波器的阶数
        filtedData = si.filtfilt(b, a, x)  # data为要过滤的信号
    elif (mode == 'lowpass'):
        b, a = si.butter(order,  Wn2, 'low')
        filtedData = si.filtfilt(b, a, x)
    elif  (mode == 'highpass'):
        b, a = si.butter(order, Wn1, 'high')
        filtedData = si.filtfilt(b, a, x)

    # 保存数据
    save_data_func(data=filtedData, output_file=output_file, save_path=save_path,
                   file_name="Fast_kurtogram",
    ...
    filtedData=np.squeeze(filtedData)
    plt.plot(filtedData)
    plt.suptitle('Fast Kurtogram', fontsize=16)

    if output_image == 0:
        file_name1 = "%s.png" % "Fast_kurtogram"
        path1 = os.path.join(save_path, file_name1)
        plt.savefig(path1)
    elif output_image == 1:
        file_name1 = "%s.png" % "Fast_kurtogram"
        file_name2 = "%s.jpg" % "Fast_kurtogram"
        path1 = os.path.join(save_path, file_name1)
        path2 = os.path.join(save_path, file_name2)
        plt.savefig(path1)
        plt.savefig(path2)
        ...
        file_name1 = "%s.png" % "Fast_kurtogram"
        file_name2 = "%s.svg" % "Fast_kurtogram"
        path1 = os.path.join(save_path, file_name1)
        path2 = os.path.join(save_path, file_name2)
        plt.savefig(path1)
        plt.savefig(path2)
        ...
        file_name1 = "%s.png" % "Fast_kurtogram"
        file_name2 = "%s.pdf" % "Fast_kurtogram"
        path1 = os.path.join(save_path, file_name1)
        path2 = os.path.join(save_path, file_name2)
        plt.savefig(path1)
        plt.savefig(path2)
...
if __name__=="__main__":
    pass
    ##输入为一维数组！！！！input_data文件来源于CRWU
    # 运行整个数据集,index为11和22的值与matlab不一致
    # x1 = writein('input_data.mat')
    # nlevel = 1  # 分解层数
    # Fs = 12000
    # fc_all = []
    # bw_all=[]
